...
export const createExercise = catchAsync(
    async (req: Request, res: Response, next: NextFunction) => {
        const { name, weight, repetitions, img_url, fk_user }: IExercise =
            req.body;
...
export const createExerciseFromRoutine = catchAsync(
    async (req: Request, res: Response, next: NextFunction) => {
...
export const findExercisesPerUser = catchAsync(
    async (req: Request, res: Response, next: NextFunction) => {
        ...
        if (!exercises.length)
            return new AppError(
                `The user with id ${id} does not yet have exercises register`,
                404
            );
...
export const findExerciseById = catchAsync(
    async (req: Request, res: Response, next: NextFunction) => {
...
export const softDeleteExercise = catchAsync(
    async (req: Request, res: Response, next: NextFunction) => {
        const exercise = (req as ExtendedExerciseRequest).exercise;
